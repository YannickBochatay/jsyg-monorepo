...
style.innerHTML = /*css*/`
    ...
    svg g.textBox rect.textSelection {
        fill:orange;
        fill-opacity:0.5;
    }
    svg g.textBox line.cursor {
        fill:none;
        stroke:black;
        stroke-width:1;
    }
...
function getComplementaryColor(node) {
    const rgb = getComputedStyle(node).fill;
    const [,r,g,b] = /rgb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)/.exec(rgb);
...
Selection.prototype.select = function(from,to) {
    ...
    jCont.fill(getComplementaryColor(node));
