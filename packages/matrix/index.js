 ...
 * @returns {Matrix}
 */
export default function Matrix(arg) {
...
Matrix.prototype = {
    ...
    decompose : function(originX,originY) {
        ...
        let rowx = new Vect(mtx.a,mtx.b);
        let scaleX = rowx.length();
        rowx = rowx.normalize();
        
        let rowy = new Vect(mtx.c,mtx.d);
        let skew = rowx.dot(rowy);
        rowy = rowy.combine(rowx, 1.0, -skew);
        
        let scaleY = rowy.length();
        rowy = rowy.normalize();
        skew /= scaleY;
        
        let rotate = Math.atan2(mtx.b,mtx.a) * 180 / Math.PI;
...
Matrix.parse = function(str) {
    ...
    const coefs = regParseMtx.exec(str);
