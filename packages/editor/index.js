...
Editor.prototype.enableCtrls = function() {
    ...
    if (nbArgs === 0 || nbArgs === 1 && arg1 == "all") {
        ...
        ctrlsToEnable = [...arguments];
    }
            
    ctrlsToEnable.forEach(function(ctrl) { that[ 'ctrls'+ ucfirst(ctrl) ].enable(); });
...
Editor.prototype.disableCtrls = function() {
    ...
    if (arguments.length === 0) {
        ...
        [...arguments].forEach(function(arg) {
            var ctrl = that['ctrls'+ ucfirst(arg) ];
...
CtrlPoints.prototype = {
     ...
     * @see StdConstruct.prototype.trigger
     */
            ...
            list.forEach(function(item,i) {
                ...
                if (test1 || test2) {
                    ...
                    if (test1) {
                        ...
                        pt1 = new Vect(seg.x1,seg.y1).mtx(CTM);
                        ...
                        drag = function(e) {
                            ...
                            pt = new Vect(center.x,center.y).mtx(jShape.getMtx(svg));
                    ...
                    if (test2) {
                        ...
                        pt1 = new Vect(seg.x2,seg.y2).mtx(CTM);
                        pt2 = new Vect(seg.x,seg.y).mtx(CTM);
        ...
        if (tag === 'path') {
            ...
            list.forEach(function(seg,i) {
                ...
                if (seg.x!=null && seg.y!=null) {
                    ...
                    var pt = new Vect(seg.x,seg.y).mtx(CTM),
                    shape,drag;
                    ...
                    if (that.list[i]) shape = new JSYG(that.list[i]);
                        ...
                        drag = function(e) {
                            ...
                            posPt = new Vect(center.x,center.y).mtx(jPoint.getMtx()), //position dans le repère d'édition
                            ...
                            if (that.strengthClosingMagnet!=null && (seg === lastSeg || seg === firstSeg)) {
                                ...
                                if (Math.sqrt(Math.pow(ref.x - e.clientX,2)+Math.pow(ref.y-e.clientY,2)) < that.strengthClosingMagnet) {
                                    ...
                                    jPoint.setCenter( new Vect(pt.x,pt.y).mtx(mtx) );
                                }
                            }
                            ...
                            if (that.autoSmooth && !that.editor.ctrlsCtrlPoints.enabled) jNode.autoSmooth(i);
                                ...
                                if (seg.x2!=null && seg.y2!=null) {
                                    ...
                                    seg.x2+=decX;
                                    seg.y2+=decY;
                                    pt1 = new Vect(seg.x2,seg.y2).mtx(selfCTM);
            ...
            list.forEach(function(point,i) {
                ...
                if (that.list[i]) shape = new JSYG(that.list[i]);
                    ...
                    drag = function(e) {
                        ...
                        selfCTM = jNode.getMtx(svg),
                        center = jPoint.getCenter(),
                        pt = new Vect(center.x,center.y).mtx(jPoint.getMtx());
            ...
            list.forEach(function(attr,i) {
                ...
                var point = new Vect(jNode.attr('x'+attr),jNode.attr('y'+attr)).mtx(CTM),
                shape,drag;
...
Drag.prototype = {
    ...
    set : StdConstruct.prototype.set,//function(opt) { return StdConstruct.prototype.set.apply(this,arguments); },
    /**
     * Ajout d'écouteurs d'événements customisés
     * @see StdConstruct.prototype.on
     * @returns {Drag}
    ...
    update : function() {
        ...
        CTM = (function() {
            if (isSVG) return jNode.getMtx(jNode.offsetParent("farthest"));
                ...
                return new Matrix().translate(dimParent.x,dimParent.y).multiply(jNode.getMtx());
            }
        }()),
        topleft = new Vect(b.x,b.y).mtx(CTM),
        topright = new Vect(b.x+b.width,b.y).mtx(CTM),
        bottomleft = new Vect(b.x,b.y+b.height).mtx(CTM),
        bottomright = new Vect(b.x+b.width,b.y+b.height).mtx(CTM),
